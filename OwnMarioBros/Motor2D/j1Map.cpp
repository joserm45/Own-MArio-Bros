...
void j1Map::Draw()
{
	...
	BROFILER_CATEGORY("Draw", Profiler::Color::Red);
...
bool j1Map::Walkability(Entity* entity)
{
	...
	{
		entity_x = (entity->position.x - 20) / 16; //check next tile left
		entity_y = (entity->position.y + 24) / 16;
	}
	else if (entity->entity_state == MOVE_RIGHT)
	{
		entity_x = (entity->position.x) / 16; //check next tile right
		entity_y = (entity->position.y) / 16;
	}
	else if (entity->entity_state == MOVE_LEFT)
	{
		entity_x = (entity->position.x - 16) / 16; //check next tile left
		entity_y = (entity->position.y) / 16;
	}
	...
	else if (entity->entity_state == MOVE_LEFT || entity->entity_state == MOVE_RIGHT)
	{
		nextGid;
		if (*nextGid == 650)
			ret = false;
